...
class KDTree
{
  ...
  public:
    int partition(vector<Point<Dim> > &currVector, int start, int end, int PivotIndex, int level);
    Point<Dim> select(vector<Point<Dim> > &currVector, int start, int end, int k, int level);
    void buildTree(vector<Point<Dim> > &currVector, int dim, int start, int end, KDTreeNode *&curRoot);
    void copyNode(KDTreeNode *otherRoot, KDTreeNode *currRoot);
