...
{
    /* Your code goes here! */
    string word1Temp = word1;
    string word2Temp = word2;

    for (auto &c1 : word1Temp)
        c1 = toupper(c1);
    for (auto &c2 : word2Temp)
        c2 = toupper(c2);

    auto word1Iter = dict.find(word1Temp);
    auto word2Iter = dict.find(word2Temp);

    if (word1Iter == dict.end() || word2Iter == dict.end())
        return false;

    vector<string> value1 = (*word1Iter).second;
    vector<string> value2 = (*word2Iter).second;
